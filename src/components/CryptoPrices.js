...
export const CryptoPrices = () => {
  ...
  const cryptoElements = useMemo(
    () =>
      cryptos.map((crypto, index) => (
        ...
        >
          <h3 className="font-bold text-white text-lg mb-2">{crypto.name}</h3>
          <p className="text-2xl text-white mb-2">${crypto.price.toFixed(2)}</p>
          <p
            className={`font-semibold ${
              parseFloat(crypto.change) >= 0 ? 'text-green-500' : 'text-red-500'
            }`}
          >
  ...
  if (loading) {
    return (
      <section className="mb-12">
        <h2 className="text-2xl font-bold mb-6 text-white">Crypto Trading</h2>
  ...
  return (
    <section className="mb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">{cryptoElements}</div>
      {lastUpdate && (
